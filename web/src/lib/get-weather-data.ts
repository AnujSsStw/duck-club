...
export async function getWeatherData(date: string, location: { lat: number; lng: number }, slot: "morning" | "mid-day" | "afternoon"): Promise<any> {
    const slotTime = timeSlot.find(t => t.slot === slot)?.avgT || "12:00";
    const weather = await fetch(
        `http://api.weatherapi.com/v1/history.json?key=${process.env.NEXT_PUBLIC_WEATHER_API}&q=${location.lat},${location.lng}&aqi=no&dt=${date.split("T")[0]}`
    );
